...
CREATE SCHEMA IF NOT EXISTS `lytsiuk` DEFAULT CHARACTER SET utf8 ;
USE `lytsiuk` ;
...
CREATE TABLE IF NOT EXISTS `lytsiuk`.`lessor` (
...
CREATE TABLE IF NOT EXISTS `lytsiuk`.`apartament` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `area` DOUBLE UNSIGNED NOT NULL,
  `adress` VARCHAR(45) NOT NULL,
  `balcony` TINYINT NULL DEFAULT NULL,
  ...
  UNIQUE INDEX `lessor_id_UNIQUE` (`lessor_id` ASC) VISIBLE,
  CONSTRAINT `fk_apartament_lessor1`
    FOREIGN KEY (`lessor_id`)
    REFERENCES `lytsiuk`.`lessor` (`id`))
ENGINE = InnoDB
AUTO_INCREMENT = 11
DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `lytsiuk`.`renter` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(20) NOT NULL,
  `surname` VARCHAR(20) NOT NULL,
  `last_name` VARCHAR(20) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `lytsiuk`.`payment_transaction` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_payment_transaction_render1_idx` (`render_id` ASC) VISIBLE,
  INDEX `fk_payment_transaction_lessor1_idx` (`lessor_id` ASC) VISIBLE,
  UNIQUE INDEX `render_id_UNIQUE` (`render_id` ASC) VISIBLE,
  UNIQUE INDEX `lessor_id_UNIQUE` (`lessor_id` ASC) VISIBLE,
  CONSTRAINT `fk_payment_transaction_lessor1`
    FOREIGN KEY (`lessor_id`)
    REFERENCES `lytsiuk`.`lessor` (`id`),
  CONSTRAINT `fk_payment_transaction_render1`
    FOREIGN KEY (`render_id`)
    REFERENCES `lytsiuk`.`renter` (`id`))
ENGINE = InnoDB
AUTO_INCREMENT = 23
DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `lytsiuk`.`apartament_reserved` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `fk_apartament_reserved_apartament`
    FOREIGN KEY (`apartament_id`)
    REFERENCES `lytsiuk`.`apartament` (`id`),
  CONSTRAINT `fk_apartament_reserved_payment_transaction1`
    FOREIGN KEY (`payment_transaction_id`)
    REFERENCES `lytsiuk`.`payment_transaction` (`id`))
ENGINE = InnoDB
AUTO_INCREMENT = 11
DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `lytsiuk`.`response` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `coment` LONGTEXT NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `lytsiuk`.`apartament_response` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_apartament_response_apartament1`
    FOREIGN KEY (`apartament_id`)
    REFERENCES `lytsiuk`.`apartament` (`id`),
  CONSTRAINT `fk_apartament_response_response1`
    FOREIGN KEY (`response_id`)
    REFERENCES `lytsiuk`.`response` (`id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `lytsiuk`.`photo` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  INDEX `fk_photo_response1_idx` (`response_id` ASC) VISIBLE,
  INDEX `fk_photo_renter1_idx` (`renter_id` ASC) VISIBLE,
  INDEX `fk_photo_lessor1_idx` (`lessor_id` ASC) VISIBLE,
  CONSTRAINT `fk_photo_response1`
    FOREIGN KEY (`response_id`)
    REFERENCES `lytsiuk`.`response` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_photo_renter1`
    FOREIGN KEY (`renter_id`)
    REFERENCES `lytsiuk`.`renter` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_photo_lessor1`
    FOREIGN KEY (`lessor_id`)
    REFERENCES `lytsiuk`.`lessor` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `lytsiuk`.`apartment_photo` (
  `photo_` INT NOT NULL,
  `apartament_id` INT NOT NULL,
  `id` INT NOT NULL AUTO_INCREMENT,
  UNIQUE INDEX `photo_id_UNIQUE` (`photo_` ASC) VISIBLE,
  INDEX `fk_apartment_photo_photo1_idx` (`photo_` ASC) VISIBLE,
  INDEX `fk_apartment_photo_apartament1_idx` (`apartament_id` ASC) VISIBLE,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_apartment_photo_apartament1`
    FOREIGN KEY (`apartament_id`)
    REFERENCES `lytsiuk`.`apartament` (`id`),
  CONSTRAINT `fk_apartment_photo_photo1`
    FOREIGN KEY (`photo_`)
    REFERENCES `lytsiuk`.`photo` (`id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
